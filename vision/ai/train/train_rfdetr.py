...
def merge_post_training_validation_metrics(run_dir: Path, payload: dict):
    ...
    if isinstance(per_class, list) and per_class:
        ...
        web_metrics["per_class_note"] = (
            "RF-DETR per-class metrics and report artifacts were generated by post-training "
            "validation inference. ROC-AUC is not generated by this backend."
        )
    ...
    for key in ("precision", "recall", "map50", "map50_95"):
        if payload.get(key) is not None:
            metric_sources.setdefault(key, "rfdetr_training.log")
    if per_class:
        metric_sources["per_class"] = "post_training_validation"
    web_metrics["metric_sources"] = metric_sources
...
def generate_rfdetr_report_artifacts(
    run_dir: Path,
    ...
    *,
    ...
    if data_yaml_path is None or not Path(data_yaml_path).is_file():
        print("RF-DETR report artifacts skipped: dataset YAML is unavailable.", flush=True)
        return
    weights_path = run_dir / "weights" / "best.pt"
    if not weights_path.is_file():
        weights_path = run_dir / "weights" / "last.pt"
    if not weights_path.is_file():
        print("RF-DETR report artifacts skipped: no normalized checkpoint was found.", flush=True)
        return

    try:
        try:
            from test_rfdetr import evaluate_rfdetr_split
        except ModuleNotFoundError:
            from vision.ai.train.test_rfdetr import evaluate_rfdetr_split

        payload = evaluate_rfdetr_split(
            weights_path,
            Path(data_yaml_path),
            run_dir,
            split="val",
        ...
        print("RF-DETR validation report artifacts saved to the training run directory.", flush=True)
    except Exception as exc:
        print(f"RF-DETR report artifacts skipped: {exc}", flush=True)
    ...
    with tee_output(run_log_path):
        ...
        try:
            ...
            finalize_rfdetr_artifacts(
                ...
                dataset_audit=dataset_audit,
                training_completed=training_completed,
                data_yaml_path=training_dataset_dir / "data.yaml",
