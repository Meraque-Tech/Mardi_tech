...
// No cv::TrackerMOSSE here: it lives in opencv2/tracking.hpp (an
// opencv_contrib module not guaranteed to be present in every OpenCV build,
// notably the one bundled with this Jetson image). This tracker only ever
// used MOSSE for a per-frame bbox *prediction* between detections; the
// actual identity/counting logic is IOU matching against each track's last
// known box, which doesn't need optical tracking at all. Dropping the
// prediction step (track boxes just hold their last detected position until
// the next match) removes the dependency with no change to the counting
// behavior.
struct TrackedObj {
    int        id;
    int        class_id;
...
public:
    ...
    std::map<int, int> update(const cv::Mat &frame, const std::vector<Detection> &dets) {
        // 1. Age every existing track by one frame; a match below resets it to 0.
        for (auto &t : tracks_) {
            t.lost++;
        }

        // 2. Match detections to existing tracks' last known boxes via IoU
