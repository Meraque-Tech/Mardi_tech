...
void AlgoUtils::updateWpNavigationMarkers(std::vector<geometry_msgs::msg::PoseStamped> &poses_)
{
    resetUniqueId();
    ...
    wp_navigation_markers_pub->publish(std::move(marker_array));
    poses_.clear();

}

void AlgoUtils::resetUniqueId()
{
  unique_id = 0;
}

int AlgoUtils::getUniqueId()
{
